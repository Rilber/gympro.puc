...
class ProjetoGYMPROSupabaseUser extends BaseAuthUser {
  ProjetoGYMPROSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> projetoGYMPROSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ProjetoGYMPROSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
